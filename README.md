...
Hangman is a classic game where the player attempts to determine the hidden word. Players are presented with blank spaces and have to individually guess letters until the blank spaces are exchanged for letters. Each wrong letter guess, results in the construction of a man hanging until the final image is drawn and the game is over. In this version, a word is randomly generated by the computer and the user has 7 chances to enter a unique letter and guess the word. Each time the user inputs an incorrect answer, a visual hangman graphic is displayed to the user to indicate the incorrect letter. 
Once the game is won, the user has the choice to play again or return to the main menu.

The link to the site can be found [here](https://hangman-p3.herokuapp.com/)

## Aims

The aim of this site is to provide a fun, interactive, easy and accessible game that delivers a challenge to the user. Users should be able to receive real time feedback as to whether their guess is correct or incorrect. Users should know whether they've won or lost the game. Users should have the ability to easily replay the game once completed.

## Planning stage
During the planning stage, I initially thought about the logic that the game must follow in order to run. I considered the step by step process from a user’s point of view and how their interaction with the game could affect the steps and logic within the game.
I considered:
* How the game would initially load
* What would be the first interaction/input from the user
* How would the user input affect the logical steps of the game

Once I considered these processes, I created a flowchart to supplement the process of building.
...
The user is then invited to input their first name. The user can only enter alphabetic characters or the game will not recognise the users input. 
...
The length of the words are then represented by underlines to show how many letters are in the word. 
...
|Input for user name | User can only enter alphabetic letters | Pass
|Welcome message | Feedback message with user's name displayed / messaged timed before game loads | Pass
...
|Guess letter input | User can only enter one alphabetic at a time or error message displayed | Pass
...
![alt text](assets/images/infinite_loop_fix.webp)
...
1. Committed and pushed the project to github
